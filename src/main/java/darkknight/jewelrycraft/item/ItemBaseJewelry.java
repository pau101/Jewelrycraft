...
public class ItemBaseJewelry extends Item
{
    ...
    public static int color(ItemStack stack, int pass) throws IOException
    {
        ...
        if (pass == 0 && stack != null && JewelryNBT.ingot(stack) != null && Item.getIdFromItem(JewelryNBT.ingot(stack).getItem()) > 0 && JewelryNBT.ingot(stack).getIconIndex() != null && JewelryNBT.ingotColor(stack) == 16777215)
        {
            ...
            if (Item.getIdFromItem(in.getItem()) == Block.getIdFromBlock(Blocks.stained_glass) || Item.getIdFromItem(in.getItem()) == Block.getIdFromBlock(Blocks.stained_hardened_clay) || Item.getIdFromItem(in.getItem()) == Block.getIdFromBlock(Blocks.wool) || Item.getIdFromItem(in.getItem()) == Block.getIdFromBlock(Blocks.carpet))
            {
                in.setItemDamage(15 - in.getItemDamage());
            }
            IIcon itemIcon = in.getItem().getIcon(in, 0);
            String ingotIconName = itemIcon.getIconName();

            if (ingotIconName.substring(0, ingotIconName.indexOf(":") + 1) != "")
            {
                domain = ingotIconName.substring(0, ingotIconName.indexOf(":") + 1).replace(":", " ").trim();
            } else
            {
                domain = "minecraft";
            }
            ...
            if (texturemanager.getResourceLocation(JewelryNBT.ingot(stack).getItemSpriteNumber()).toString().contains("items"))
            {
                ingot = new ResourceLocation(domain.toLowerCase(), "textures/items/" + texture);
            } else
            {
                ingot = new ResourceLocation(domain.toLowerCase(), "textures/blocks/" + texture);
            }
            ...
            if (JewelryNBT.ingot(stack) != null && JewelryNBT.ingot(stack).getItem().getColorFromItemStack(JewelryNBT.ingot(stack), 1) != 16777215)
            {
                JewelryNBT.addIngotColor(stack, JewelryNBT.ingot(stack).getItem().getColorFromItemStack(JewelryNBT.ingot(stack), 1));
            } else
            {
                JewelryNBT.addIngotColor(stack, color);
            }
        } else if (pass == 1 && stack != null && JewelryNBT.gem(stack) != null && JewelryNBT.gem(stack).getIconIndex() != null && JewelryNBT.gem(stack) != null)
        {
            ...
            if (jewelIconName.substring(0, jewelIconName.indexOf(":") + 1) != "")
            {
                domain = jewelIconName.substring(0, jewelIconName.indexOf(":") + 1).replace(":", " ").trim();
            } else
            {
                domain = "minecraft";
            }
            ...
            if (JewelryNBT.gem(stack).getUnlocalizedName().contains("item"))
            {
                jewelLoc = new ResourceLocation(domain, "textures/items/" + texture);
            } else
            {
                jewelLoc = new ResourceLocation(domain, "textures/blocks/" + texture);
            }
            ...
            int color = getMostCommonColour(m);
            if (JewelryNBT.gem(stack).getItem().getColorFromItemStack(JewelryNBT.gem(stack), 1) == 16777215)
            {
                JewelryNBT.addGemColor(stack, color);
            } else
            {
                JewelryNBT.addGemColor(stack, JewelryNBT.gem(stack).getItem().getColorFromItemStack(JewelryNBT.gem(stack), 1));
            }
        }
        if (pass == 0 && JewelryNBT.ingot(stack) != null)
        {
            return JewelryNBT.ingotColor(stack);
        }
        if (pass == 1 && JewelryNBT.gem(stack) != null)
        {
            return JewelryNBT.gemColor(stack);
    ...
    public String getItemStackDisplayName(ItemStack stack)
    {
        ...
        {
            return JewelryNBT.ingot(stack).getDisplayName().replace("Ingot", " ").trim() + " " + ("" + StatCollector.translateToLocal(getUnlocalizedNameInefficiently(stack) + ".name")).trim();
        }
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean par4)
    {
        if (stack.hasTagCompound() && par4)
        {
            ...
            {
                list.add("Ingot: " + EnumChatFormatting.YELLOW + ingot.getDisplayName());
            }

            ItemStack gem = JewelryNBT.gem(stack);
            if (gem != null)
            {
                list.add("Gem: " + EnumChatFormatting.BLUE + gem.getDisplayName());
            }
